        ...
        public static async void setCpuFamily()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(processorIdentifier))
                {
                    ...
                    int familyIndex = Array.IndexOf(words, "Family");
                    if (familyIndex >= 0 && familyIndex + 1 < words.Length) int.TryParse(words[familyIndex + 1], out CPUFamily);

                    int modelIndex = Array.IndexOf(words, "Model");
                    if (modelIndex >= 0 && modelIndex + 1 < words.Length) int.TryParse(words[modelIndex + 1], out CPUModel);

                    int steppingIndex = Array.IndexOf(words, "Stepping");
                    if (steppingIndex >= 0 && steppingIndex + 1 < words.Length) int.TryParse(words[steppingIndex + 1].TrimEnd(','), out CPUStepping);
                }
                ...
                try
                {
                    ...
                    {
                        if (mo["Name"] != null) CPUName = mo["Name"].ToString();
                    }
                }
                catch { }
                ...
                {
                    CPUName = System.Environment.GetEnvironmentVariable("PROCESSOR_IDENTIFIER") ?? "x86-64 Processor";
                }
            }
            catch (Exception e)
            {
