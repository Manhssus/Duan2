...
namespace Universal_x86_Tuning_Utility.Scripts.Intel_Backend
{
    public static class Intel_Management
    {
        ...
        public static string BaseDir => string.IsNullOrEmpty(Settings.Default.Path) ? AppContext.BaseDirectory : Settings.Default.Path;
        ...
        public static void changePowerLimits(int pl1, int pl2)
        {
            try
            {
                EnsureInitialised();
                runIntelTDPChangeMSR(pl1, pl2);
            }
            catch (Exception ex)
            {
                Misc.DiagnosticLogger.LogError(ex, "Failed to change Intel PL1/PL2 MSR.");
            }
        }
        ...
        private static string? ResolveIntelModulePath(string fileName)
        {
            ...
            string[] candidates =
            {
                Path.Combine(dir, "Assets", "Intel", "PawnIO", binName),
                Path.Combine(dir, "Assets", "Intel", "PawnIO", fileName),
                Path.Combine(AppContext.BaseDirectory, "Assets", "Intel", "PawnIO", binName),
                Path.Combine(AppContext.BaseDirectory, "Assets", "Intel", "PawnIO", fileName),
                binName,
                fileName
            };
