    ...
    internal class PremadePresets
    {
        ...
        public static void SetPremadePresets()
        {
            try
            {
                ...
                {
                    cpuName = Family.CPUName ?? "";
                    uri = new Uri("pack://application:,,,/Assets/config.png");

                    if (cpuName.Contains("U") || cpuName.Contains("Y"))
                    {
                        // 15W U-Series (e.g., i5-1235U, i7-1355U, etc.)
                        EcoPreset = "--intel-pl=12 --Win-Power=0 ";
                        BalPreset = "--intel-pl=15 --Win-Power=1 ";
                        PerformancePreset = "--intel-pl=25 --Win-Power=2 ";
                        ExtremePreset = "--intel-pl=35 --Win-Power=2 ";
                    }
                    else if (cpuName.Contains("P") || cpuName.Contains("H") || cpuName.Contains("HX") || cpuName.Contains("HK"))
                    {
                        // 28W-45W+ Performance / Gaming Mobile
                        EcoPreset = "--intel-pl=20 --Win-Power=0 ";
                        BalPreset = "--intel-pl=35 --Win-Power=1 ";
                        PerformancePreset = "--intel-pl=55 --Win-Power=2 ";
                        ExtremePreset = "--intel-pl=75 --Win-Power=2 ";
                    }
                    else
                    {
                        // Desktop or other Intel
                        EcoPreset = "--intel-pl=15 --Win-Power=0 ";
                        BalPreset = "--intel-pl=28 --Win-Power=1 ";
                        PerformancePreset = "--intel-pl=45 --Win-Power=2 ";
                        ExtremePreset = "--intel-pl=65 --Win-Power=2 ";
                    }
                }
            }
            catch (Exception ex)
